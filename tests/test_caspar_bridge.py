...
class FakeGateway:
    ...
    def _run(self):
        ...
        while True:
            ...
            if opcode == OP_HELLO:
                self.hello = value
                self._send(OP_WELCOME, corr, {"ok": True, "sessionId": 7})
...
def test_handshake_and_call(gateway):
    client = CasparBridgeClient("127.0.0.1", gateway.port, vm_id="vm1", program_id="prog1", timeout=5)
    client.connect()
...
def test_large_payload_chunks(gateway):
    client = CasparBridgeClient("127.0.0.1", gateway.port, vm_id="vm1", timeout=10)
    client.connect()
...
def test_signal_delivery(gateway):
    received = []
    client = CasparBridgeClient("127.0.0.1", gateway.port, vm_id="vm1", timeout=5)
