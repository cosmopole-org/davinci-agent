...
TASK_OBJECTIVE = (
    ...
    "tool always returns the fully rendered page text in its result's "
    "'final.text' field and the rendered HTML in 'final.content_excerpt' — read "
    "the posts from there. If a CSS selector you guess does not match, do NOT "
    "abort or fall back to dummy data: parse the real posts straight out of the "
    "returned 'final.text'. Also collect the URLs of any chart screenshot images "
    "attached to the posts and analyse those charts for the predicted levels.\n\n"
    ...
    "point).\n\n"
    "3) FETCH THE REAL PRICES. For every (crypto, predicted date/time) pair, "
    "fetch the ACTUAL historical market price at that timestamp using the "
    "python-execution tool with the 'requests' library against a real historical "
    "crypto-price API (for example the public CoinGecko API). Use real fetched "
    "data only — never invent, assume, or use placeholder/dummy prices. If a "
    "price lookup fails, retry with another endpoint rather than fabricating a "
    "value.\n\n"
    "4) SCORE EACH PREDICTION. Give each prediction a score from 0 to 100 from "
    "how close the predicted price was to the real price at that time — e.g. "
    "score = max(0, 100 - absolute_percentage_error). Do the arithmetic in the "
    "python-execution tool.\n\n"
    "5) AVERAGE & OUTPUT. Compute the mean of all the individual scores. Your "
    "final answer MUST be ONLY that single number (a value between 0 and 100) "
    "and nothing else — no words, units, or explanation."
)
